...
There is a staging version of testhub deployed here (http://134.122.102.159:30001/)

There is a jenkins pipeline configured [here](http://134.122.102.159:30000/) to run this job against the staging testhub and to generate test reports. It's named `test-examples`. You can access it with the below credentials
...
Staging
```
curl -v -XPOST -H 'Content-Type:application/json' -d @build/cucumber-reports.json http://134.122.102.159:30001/reports/test-examples
```
