...
There is a staging version of testhub deployed here (http://134.209.180.137:30001/)

There is a jenkins pipeline configured [here](http://134.209.180.137:30000) to run this job against the staging testhub and to generate test reports. It's named `test-examples`. You can access it with the below credentials
...
Staging
```
curl -v -XPOST -H 'Content-Type:application/json' -d @build/cucumber-reports.json http://138.68.170.14:30001/reports/test-examples
```
